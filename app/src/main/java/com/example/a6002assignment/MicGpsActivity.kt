...
class MicGpsActivity : AppCompatActivity(), LocationListener {
    ...
    override fun onLocationChanged(p0: Location) {
        ...
        uploadGpsButton.setOnClickListener {
            //Use the setValue method to write latitude, longitude and accress to the specific location of the firebase realtime database,
            //uid and key Get from CameraActivity
            var database: DatabaseReference = Firebase.database.reference
            ...
            database.child("user").child(uid.toString()).child(key.toString()).child("address")
                .setValue(address)
    ...
    fun gotoMain(view: View) {
        ////When the button is clicked, the user ID will be sent to MainActivity
        val uid = intent.getStringExtra("uid")
