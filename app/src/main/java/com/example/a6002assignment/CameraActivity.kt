...
class CameraActivity : AppCompatActivity() {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == REQUEST_CODE_CAMERA) {
            //When the user takes a photo, the photo data will be displayed in the view in bitmap format
            val photo = data?.extras?.get("data") as Bitmap
            photoView.setImageBitmap(photo)
            //Create a reference to a photo for firebase storage
            val reference = storageRef.child(photo.toString())

            //Get bitmap and get the data from an photoView as bytes
            //enable drawing cache
            photoView.setDrawingCacheEnabled(true)
            //If the drawing cache is invalid, the drawing cache is forced to be generated
            photoView.buildDrawingCache()
            //Returns the bitmap in which this view drawing is cached
            val bitmap = photoView.drawingCache
            //Create a buffer of byte type arrays
            val baos = ByteArrayOutputStream()
            //Compress bitmap storage size
            bitmap.compress(Bitmap.CompressFormat.JPEG, 100, baos)
            //instantiate a new buffer of byte type, stored in data
            val data = baos.toByteArray()

            //upload photo
            val uploadButton: Button = findViewById(R.id.btn_upload)
            uploadButton.setOnClickListener {
                //When the uploadButton is clicked, data will be uploaded to Firebase Cloud Storage using the putBytes() method
                ...
                uploadTask.addOnFailureListener {
                    ...
                    .addOnSuccessListener { taskSnapshot ->
                        //When the upload is successful, use the getDownloadUrl() method from
                        // firebase Storage to obtain the url of the uploaded photo
                        ...
                        task.addOnSuccessListener { uri ->
                            ...
                            //Use the push and setValue method to write photoUri to the
                            // specific location of the firebase realtime database,
                            ...
                            //and use the setValue method to push key to the specific location of the firebase realtime database
                            var key = databseReference.key
                            databseReference.child("key").setValue(key)
